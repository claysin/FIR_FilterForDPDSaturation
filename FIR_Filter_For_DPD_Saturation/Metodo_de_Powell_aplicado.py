...
resposta_ao_impulso_LTE = np.real(np.fft.fftshift(np.fft.ifft(np.fft.ifftshift(normalizado_LTE))))
resposta_ao_impulso_wifi = np.real(np.fft.fftshift(np.fft.ifft(np.fft.ifftshift(normalizado_wifi))))
...
def funcao_objetivo(coeficientes, sinal_wifi, sinal_LTE, Fs, delta_w, L, envoltoria_sinal_saturado):
    ...
    # métrica
    erro = np.mean((np.abs(xf2) - np.abs(envoltoria_sinal_saturado)) ** 2)
    return erro

# ================== OTIMIZAÇÃO ==================
coef_iniciais = (resposta_ao_impulso_wifi[:9] + resposta_ao_impulso_LTE[:9]) / 2.0


resultado = minimize(
    funcao_objetivo,
    coef_iniciais,resposta_ao_impulso_LTE,
